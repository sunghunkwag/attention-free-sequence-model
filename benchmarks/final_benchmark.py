"""
Final benchmark: AFN v3 vs Transformer — param-matched (~103-105K).

Three synthetic multi-scale tasks:
  1. Nested Depth: bracket depth tracking (local rules + hierarchical aggregation)
  2. MultiScale Copy: reproduce a pattern 32 positions away (long-range content access)
  3. Hierarchical Parity: group parity + meta-parity (local computation + global reduction)
...
# =========================================================================
# Transformer baseline (minimal, param-matched)
# =========================================================================
...
def train_and_evaluate(model, task_fn, n_steps=200, batch_size=16,
                       lr=3e-4, ignore_index=-100):
...
def main():
    ...
    print()
    print("=" * 90)
    print("AFN v3 vs TRANSFORMER — PARAM-MATCHED, ZERO ATTENTION")
    print("=" * 90)
    ...
    for task_name, task_fn, vocab_out, ignore_idx, n_steps in tasks:
        ...
        for model_name, model in [('Transformer', transformer), ('AFN v3', afn3)]:
            torch.manual_seed(42)
            loss, acc, dt = train_and_evaluate(
                model, task_fn, n_steps=n_steps, ignore_index=ignore_idx)
            print(f"  {model_name:<15} loss={loss:.4f}  acc={acc:.4f}  time={dt:.1f}s")
    ...
    for task, model, params, loss, acc, dt in results:
        ...
        print(f"{task:<20} {model:<15} {params:>10,} {loss:>10.4f} "
              f"{acc:>10.4f} {dt:>7.1f}s{marker}")

    # Win count
    afn_wins = sum(1 for task in ['Nested Depth', 'MultiScale Copy', 'Hier. Parity']
                   if max((r[4], r[1]) for r in results if r[0] == task)[1] == 'AFN v3')
    print(f"\nAFN v3 wins: {afn_wins}/3 tasks")
